...
git diff --name-status HEAD template/main -- \
  ...
  .github/workflows/dependency-review.yml scripts/check-toolchain-age.ts \
...
| Security and CI   | `scripts/secret-scan.sh`, `.github/workflows/ci.yml`, `.github/workflows/codeql.yml`, `.github/workflows/dependency-review.yml` | Existing deployment, release, preview, and environment-specific jobs; if the destination repository is private and has a GitHub Advanced Security licence, set the `GHAS_ENABLED` repository variable to `true` to turn on CodeQL and Dependency Review (they are no-ops on private repos without it) |
